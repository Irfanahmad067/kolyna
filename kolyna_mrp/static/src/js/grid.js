...
const MrpProductionSchedulerGrid = AbstractAction.extend({
    contentTemplate: 'productionSchedulerGrid',
    ...
    willStart: function() {
        return Promise.all([this._super.apply(this, arguments), this.fetch_matrix_data()]);
    },
    ...
    re_renderElement: function() {
        var self = this;
        var $content = QWeb.render('productionSchedulerGrid', {widget: this});
        $('.o_form_view').replaceWith($content);
        self._renderButtons();
        self._updateControlPanel();
    },
    ...
    _renderButtons: function() {
        ...
        this.$buttons.filter('button.attribute_menu').on('click', this._onClickAttributeMenu.bind(this));
        return this.$buttons;
    },
    ...
    _updateControlPanel: function() {
        ...
        this.$searchview_buttons.find('.dropdown-menu').bind('click', function(ev){
            ev.stopPropagation()
        });
    ...
    _saveGridData: function(createmo=false) {
        ...
        this._rpc({
            ...
            self.displayNotification({ message: res });
            self.unsetRations();
        });
    },
    ...
    _onClickAttributeMenu: function(ev) {
        var self = this;
...
core.action_registry.add('mrp_production_scheduler_grid', MrpProductionSchedulerGrid);
return MrpProductionSchedulerGrid
});
